...
SHA-256, canonical structured runner argv, commit-bound timing, each
capability-specific profile, transport stack and enabled-run scenario, and
unique SHA-256-verified evidence files for its directions. Native QUIC, native
TCP/Yamux and private TCP/Yamux+pnet are separate proofs; an optional capability
is tested only through an enabled run and is never thereby claimed as default.
...
`FORGE_P2P_STAGE6_ACCEPTANCE_ARTIFACT`, so PR0 does not require a live run.
