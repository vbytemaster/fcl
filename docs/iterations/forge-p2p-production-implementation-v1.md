...
current runner registrations; it never emits an execution PASS. A live
acceptance PASS requires a separate artifact matching
`interop_acceptance_registry.artifact_schema`, generated by the external runner
at the exact reviewed `HEAD` and a clean tracked tree. It records the manifest
path/SHA-256, the recorded `[sys.executable, *sys.argv]`, commit-bound timing,
start/end clean worktree identities, fixture `build-info`, raw runner records
and a relative SHA-256 evidence index for their logs/results. The acceptance
checker does not accept a second capability-result or hand-authored-proof format:
it derives every direction from the dialer/listener raw record, requires every
command attempt and listener to terminate with `exit_code=0`, and rejects a
reused evidence file. Native QUIC, native TCP/Yamux and private TCP/Yamux+pnet
are separate proofs; an optional capability is tested only through an enabled
run and is never thereby claimed as default.
...
PR0 does not require a live artifact. Promotion is explicit and fails `NOT_RUN`
when the cache path is unset or missing:

```bash
cmake -S . -B build/forge-p2p-stage6 -G Ninja \
  -DFORGE_P2P_STAGE6_ACCEPTANCE_ARTIFACT=/absolute/path/to/interop-artifacts.json
